...
std::string run(FileReader& reader, int connections=1000) {
    ...
    std::sort(circuits.begin(), circuits.end(), [](const std::vector<std::string>& a, const std::vector<std::string>& b) {
        return a.size() > b.size();
    });

    return std::to_string(circuits[0].size() * circuits[1].size() * circuits[2].size());
}
